...
DWORD CALLBACK web_stream::client_thread(void* _p)
{
	...
	sprintf_s(buffer, response, boundary);
...
void convertBmpToJpeg(int width,int height,char* buffer) {

	int bit = 32;
	int quality = 80; ////压缩质量
	int line_stride = width * 4;
	...
	while (cinfo.next_scanline < cinfo.image_height) {
		byte* line = (byte*)buffer + cinfo.next_scanline * line_stride;
